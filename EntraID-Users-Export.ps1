...
# === Step 3: Filter out displayName starting with "xx " ===
$filteredUsers = $allUsers | Where-Object { -not ($_.displayName -like "xx *") }

# === Step 4: Save to CSV ===
New-Item -ItemType Directory -Path $tempFolder -Force | Out-Null
$filteredUsers | Select-Object id, displayName, userPrincipalName | Export-Csv -Path $tempFile -NoTypeInformation -Force

if (-not (Test-Path $tempFile)) {
    throw "❌ CSV file not created: $tempFile"
}

# === Step 5: Get access token for Azure Storage ===
...
# === Step 6: Upload CSV to Azure Blob Storage ===
$uploadHeaders = @{
